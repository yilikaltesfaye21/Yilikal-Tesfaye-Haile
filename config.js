// User customizable configuration
export const name = " Yilikal Tesfaye Haile";
export const initials = "YH";
export const tagline = "Statistical Consultant & Research Scientist";

// Color scheme - updated to calm blues and greens
export const primaryColor = "#3a7ca5";
export const accentColor = "#81a4cd";
export const darkColor = "#2c4a63";

// Social media links - update with your own URLs
export const linkedinUrl = "https://www.linkedin.com/in/yilikal-tesfaye-a1565b110/";
export const githubUrl = "#";
export const researchGateUrl = "https://www.researchgate.net/profile/Yilikal-Haile?ev=hdr_xprf";

// Contact information - update with your own details
export const email = "yilikal.tesfaye@amu.edu.et";
export const phone = "+251 910-83-84-99";
export const location = "Arba Minch, Ethiopia";

// CV Download link - update with your own CV file URL
export const cvUrl = "https://example.com/cv.pdf";

// Email.js configuration - update with your own service credentials
export const emailjsServiceId = "service_id";
export const emailjsTemplateId = "template_id";
export const emailjsUserId = "user_id";
